  ...
  rfl_drafts_with_elo %>%
    ...
    dplyr::mutate(
      hr = dplyr::case_when(
        ...
        season > (max(season) - 2) & season < max(season) & top8 >= 1 ~ 1, # 1 Top8 für junge spieler
      ...
      hit = dplyr::case_when(
        ...
        season > (max(season) - 2) & pos_grouped %in% c("QB", "TE", "PK") & top12 >= 1 ~ 1, # Top12 für junge spieler
        ...
        season > (max(season) - 2) & pos_grouped %in% c("RB", "WR", "DL", "LB", "DB") & top24 >= 1 ~ 1, # 1 Top24 für junge spieler
        TRUE ~ 0
      ),
      miss = dplyr::case_when(
        season < (max(season) - 2) & pos_grouped %in% c("QB", "TE", "PK") & top24 == 0 ~ 1, # kein top 24 finish
        season < (max(season) - 2) & pos_grouped %in% c("RB", "DL", "DB") & top48 == 0 ~ 1, # kein top 48 finish
        season < (max(season) - 2) & pos_grouped %in% c("WR", "LB") & top60 == 0 ~ 1, # kein top 48 finish
        TRUE ~ 0
      )
    )
})
