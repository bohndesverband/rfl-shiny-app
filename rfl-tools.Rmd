...
var.seasonBeforeWeek1 <- var.season
var.seasonBeforeWeek2 <- var.season

if (var.week == 1) {
  var.seasonBeforeWeek1 <- nflreadr::get_current_season() - 1
}

if (nflreadr::get_current_week(TRUE) < 2) {
  var.seasonBeforeWeek2 <- nflreadr::get_current_season() - 1
}
...
var.posOrder <- c("QB", "RB", "WR", "TE", "FLX", "PK", "DL", "LB", "DB", "IDP")
...
source("R/true-standing/rfl-true-standing.R")
...
renderPlot({
  running_elo %>% 
    ggplot(aes(x = game, y = franchise_elo_postgame, color = franchise_name)) +
      plotDefaults +
      geom_hline(yintercept = 1500, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # default elo
      
      geom_vline(xintercept = 13, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # start 2017
      geom_text(label = "2017", x = 13, y = 1300, color = var.colorRed, size = 3.5) +
    
      geom_vline(xintercept = 25, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # start 2018
      geom_text(label = "2018", x = 25, y = 1300, color = var.colorRed, size = 3.5) +
    
      geom_vline(xintercept = 37, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # start 2019
      geom_text(label = "2019", x = 37, y = 1300, color = var.colorRed, size = 3.5) +
    
      geom_vline(xintercept = 49, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # start 2020
      geom_text(label = "2020", x = 49, y = 1300, color = var.colorRed, size = 3.5) +
    
      geom_vline(xintercept = 61, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # start 2021
      geom_text(label = "2021", x = 61, y = 1300, color = var.colorRed, size = 3.5) +
    
      geom_vline(xintercept = 74, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # start 2022
      geom_text(label = "2022", x = 74, y = 1300, color = var.colorRed, size = 3.5) +
    
      geom_vline(xintercept = 87, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # start 2023
      geom_text(label = "2023", x = 87, y = 1300, color = var.colorRed, size = 3.5) +
    
      geom_vline(xintercept = 100, color = var.colorRed, linewidth = 0.5, alpha = 0.75) + # start 2024
      geom_text(label = "2024", x = 100, y = 1300, color = var.colorRed, size = 3.5) +
      ...
      scale_x_discrete(limits = c(1, 113))
})
```
...
DT::renderDataTable({
  table_data <- elo %>%
    ...
    dplyr::arrange(week) %>% 
    dplyr::mutate(
      franchise_elo_pregame = ifelse(week == 1, franchise_elo_pregame, dplyr::lag(franchise_elo_pregame)), # setze pregame elo der letzten woche auf die der ersten
...
sliderInput("warMinStarts", "Minimum Starts eines Spielers", min = 0, max = 13, value = ceiling(var.week / 2), sep = "")
```
...
sos <- read.csv(paste0("https://raw.githubusercontent.com/bohndesverband/rfl-data/main/data/schedule/rfl-sos-", var.seasonNewYear, ".csv"),
...
# Transaction History {data-navmenu="Spieler"}

## Sidebar {.sidebar}
```{r inputs-player-transaction-history, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
source("R/transactions/player-transaction-history.R", local = T)

selectizeInput(
  "playerTransactionSelectTeam",
  "Filter nach Team",
...
playerList <- reactive({
  player_transaction_history %>% 
    dplyr::filter(franchise_id == input$playerTransactionSelectTeam)
})

#selectizeInput(
#  "playerTransactionSelectPlayer",
#  "Filter nach Spielern",
#  choices = setNames(playerList()$player_identifier, playerList()$player_identifier),
#  multiple = false,
#  options = list(
#    placeholder = "Spieler wählen",
#    onInitialize = I('function() { this.setValue(""); }')
...
```{r player-transaction-history, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
plot_data <- flow %>% 
  dplyr::filter(player_identifier == player_id)
...
trade_history <- reactive({
  trades %>% 
    ...
    dplyr::filter(
      ...
      dplyr::filter(
        date >= input$tradeHistoryDate[1] & date <= (input$tradeHistoryDate[2] + 1)
      ) %>%
      dplyr::filter(
...
sliderInput("draftEloSelectSeason", "Saison", min = 2017, max = var.season, value = c(2017, var.season), sep = "", step = 1)
...
sliderInput("draftHitRatesSelectSeason", "Saison", min = 2017, max = var.season, value = c(2017, var.season - 2), sep = "", step = 1)
...
renderPlot({
  draft_hit_rates() %>%
    ...
    ggplot2::scale_color_manual(values = c(var.colorYellow, "#10ac84", "#ee5253")) +
    ggplot2::scale_fill_manual(values = c(var.colorYellow, "#10ac84", "#ee5253"), guide = "none") +
  
    ggplot2::geom_point(data = subset(team_hit_rates(), category == "Hit" & franchise_id == input$draftHitRatesSelectTeam), group = 1, position = position_nudge(x = -0.3), color = var.colorAccent, size = 12) +
    ggplot2::geom_text(data = subset(team_hit_rates(), category == "Hit" & franchise_id == input$draftHitRatesSelectTeam), ggplot2::aes(label = paste(picks, total_picks_in_round, sep = "/")), position = position_nudge(x = -0.3), size = 5, show.legend = FALSE, fontface = "bold") +
    
    ggplot2::geom_point(data = subset(team_hit_rates(), category == "Home Run" & franchise_id == input$draftHitRatesSelectTeam), group = 1, color = var.colorAccent, size = 12) +
    ggplot2::geom_text(data = subset(team_hit_rates(), category == "Home Run" & franchise_id == input$draftHitRatesSelectTeam), ggplot2::aes(label = paste(picks, total_picks_in_round, sep = "/")), size = 5, show.legend = FALSE, fontface = "bold") +
    
    ggplot2::geom_point(data = subset(team_hit_rates(), category == "Miss" & franchise_id == input$draftHitRatesSelectTeam), group = 1, position = position_nudge(x = 0.3), color = var.colorAccent, size = 12) +
    ggplot2::geom_text(data = subset(team_hit_rates(), category == "Miss" & franchise_id == input$draftHitRatesSelectTeam), ggplot2::aes(label = paste(picks, total_picks_in_round, sep = "/")), position = position_nudge(x = 0.3), size = 5, show.legend = FALSE, fontface = "bold") +
    ...
    ggplot2::labs(
      ...
      subtitle = "Balken = Ligadurchschnitt, Punkte = Franchises (Text im Kreis = Anzahl an Picks in Runde)",
      x = "Draft Round",
      y = "",
      color = ""
    ) +
...
# Reaches/Steals {data-navmenu="Draft"}

## Inputs {.sidebar}

```{r inputs-draft-reaches-steals, echo=FALSE, message=FALSE, warning=FALSE}
source("R/drafts/rfl-draft-steals-reaches.R")
```

Die Berechnung von Steals und Reaches ist die Differenz des eigenen Picks zum ersten oder letzten Pick geteilt durch den Logarithmus des eigenen Picks + 1, um den Picks in den Späteren Runden weniger Gewicht zu geben.
...
renderPlot({
  ggplot(biggest_draft_steals, aes(y = reorder(label, desc(score)), x = score)) +
    geom_bar(stat = "identity", fill = var.colorAccent) +
    coord_flip() +
    labs(
      title = "Größte Draft Steals",
      x = "",
      y = "",
    ) +
    theme(
...
renderPlot({
  ggplot(biggest_draft_reaches, aes(y = reorder(label, score), x = score)) +
    geom_bar(stat = "identity", fill = var.colorAccent) +
    coord_flip() +
    labs(
      title = "Größte Draft Reaches",
      x = "",
      y = "",
    ) +
    theme(
...
```{r draft-reaches-steals, echo=FALSE, fig.height=8, message=FALSE, warning=FALSE}

plotly::renderPlotly({
   ggplotly(biggest_draft_diff_plot, tooltip = c("text", "min_pick", "max_pick", "pick_diff"))
})
```
